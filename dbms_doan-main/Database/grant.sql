/*
Grant-based security setup for BookstoreDB
Run this script in SSMS (or sqlcmd) as a sysadmin / db_owner ONCE.
It creates two database roles:
  app_admin   - full access via stored procedures + maintenance
  app_manager - limited access (no role/user maintenance)

Two SQL logins + mapped users (optional for demo):
  login: admin_login   password: AdminLogin@123
  login: manager_login password: ManagerLogin@123

Application pattern:
 - App authenticates user against internal users table (already implemented)
 - After determining role (Admin / Manager) it OPENS a new SqlConnection using the
   corresponding SQL login so that database engine itself enforces GRANT rules.
 - UI hiding tabs still kept for better UX, but security is now also enforced at DB level.

If you prefer Windows Authentication per user, skip the CREATE LOGIN parts and map existing
Windows logins to database users then add them to the database roles (EXEC sp_addrolemember ...).
*/
USE BookstoreDB;
GO

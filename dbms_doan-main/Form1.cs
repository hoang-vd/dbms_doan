...
namespace QuanLyNhanVien
{
    ...
    private readonly UserSession _session; // current logged in user
        ...
        {
            // Manager cũng có quyền với tab Vai trò; chỉ những ai không phải Admin hoặc Manager mới bị khóa
            ...
            // Tab Admin chỉ dành cho Admin thực sự
        ...
        {
            // Chặn truy cập tab Vai trò nếu không phải Admin hoặc Manager
            if (e.TabPage == this.tabPageRole && !(IsAdmin || IsManager))
            {
                MessageBox.Show("Chỉ Admin hoặc Manager mới được truy cập tab Vai trò.", "Phân quyền", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
                return;
            }
            // Chặn truy cập tab Admin nếu không phải Admin
            if (e.TabPage == this.tabPageAdmin && !IsAdmin)
            {
                MessageBox.Show("Chỉ Quản trị viên (Admin) mới được truy cập tab Admin.", "Phân quyền", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
            }
        }
        ...
        private void LoadRolesGrid()
        {
            ...
            {
                using (var conn = new SqlConnection(connectionString))
                using (var cmd = new SqlCommand("sp_GetRolesForGrid", conn))
                using (var da = new SqlDataAdapter(cmd))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void LoadEmployees()
        {
            ...
            {
                using (var conn = new SqlConnection(connectionString))
                using (var cmd = new SqlCommand("sp_GetEmployees", conn))
                using (var da = new SqlDataAdapter(cmd))
                {
                    // NOTE: sp_GetEmployees now (or can be updated to) SELECT * FROM dbo.vw_Employees.
                    // This keeps UI code unchanged while allowing the SELECT shape to be managed in view.sql.
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void ClearUserAccountForm()
        {
            ...
            // default active checked for new user
            if (chbIsActive != null) chbIsActive.Checked = true;
        }
        ...
        private void SetUserAccountFormState(bool editing)
        {
            ...
            if (chbIsActive != null) chbIsActive.Enabled = editing; // allow toggle when editing
        }
        ...
        private void DataGridViewAdmin_SelectionChanged(object sender, EventArgs e)
        {
            ...
            // Active state
            if (chbIsActive != null && dataGridViewAdmin.Columns.Contains("is_active") && row.Cells["is_active"].Value != DBNull.Value)
            {
                try { chbIsActive.Checked = Convert.ToBoolean(row.Cells["is_active"].Value); } catch { chbIsActive.Checked = true; }
            }
        ...
        private bool RequireAdmin()
        {
            ...
            if (!(IsAdmin || IsManager))
            {
                MessageBox.Show("Chỉ Admin hoặc Manager mới được thực hiện thao tác này.", "Phân quyền", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
            return true;
        }
        ...
        private void LoadEmployeesForShift()
        {
            ...
            {
                using (var conn = new SqlConnection(connectionString))
                using (var cmd = new SqlCommand("sp_GetEmployeesForShift", conn))
                using (var da = new SqlDataAdapter(cmd))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    var dt = new DataTable();
        ...
        private void LoadShiftsByDate()
        {
            ...
            {
                using (var conn = new SqlConnection(connectionString))
                using (var cmd = new SqlCommand("sp_GetShiftsByDate", conn))
                using (var da = new SqlDataAdapter(cmd))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@shift_date", date);
            ...
            try
            {
                ...
                using (var da = new SqlDataAdapter(cmd))
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        row["display"] = string.Format("{0} - {1}", row["id"], row["name"]);
                    }
                    var r = dt.NewRow();
                    r["id"] = DBNull.Value;
                    r["name"] = "(Chưa chọn)";
                    ...
                    cmbEmployeeId.DisplayMember = "display";
        ...
        private void LoadUsersGrid4()
        {
            try
            {
                ...
                using (var da = new SqlDataAdapter(cmd))
                {
                    ...
                    if (dataGridViewAdmin.Columns.Contains("last_login")) dataGridViewAdmin.Columns["last_login"].HeaderText = "Đăng nhập lần cuối";
                }
            }
        ...
        private void InitializeShiftTabDefaults()
        {
            ...
            // Populate shift status (kinh nghiệm) combo box: values follow DB constraint ('Mới','Lâu năm')
            if (cmbExperience != null)
            {
                cmbExperience.Items.Clear();
                cmbExperience.Items.Add("Mới");
                cmbExperience.Items.Add("Lâu năm");
                cmbExperience.SelectedIndex = 0; // default
            }
        }
        ...
        private void ClearShiftForm()
        {
            ...
            if (cmbExperience != null && cmbExperience.Items.Count > 0) cmbExperience.SelectedIndex = 0; // default 'Mới'
        }
        ...
        private void SetShiftFormState(bool editing)
        {
            ...
            if (cmbExperience != null) cmbExperience.Enabled = editing;            
        }
        ...
        private void DgvShifts_SelectionChanged(object sender, EventArgs e)
        {
            ...
            // Load status -> cmbExperience
            if (cmbExperience != null && dgvShifts.Columns.Contains("status"))
            {
                var statusVal = row.Cells["status"].Value?.ToString();
                if (!string.IsNullOrEmpty(statusVal))
                {
                    int idx = cmbExperience.Items.IndexOf(statusVal);
                    if (idx >= 0) cmbExperience.SelectedIndex = idx;
                }
            }
        ...
        private void btnDelete3_Click(object sender, EventArgs e)
        {
            ...
            {
                using (var conn = new SqlConnection(connectionString))
                using (var cmd = new SqlCommand("sp_DeleteShift", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@id", id);
                    conn.Open();
        ...
        private void btnSave3_Click(object sender, EventArgs e)
        {
            ...
            if (cmbExperience != null)
            {
                ...
                {
                    MessageBox.Show("Chọn trạng thái (Mới / Lâu năm).", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                statusValue = cmbExperience.SelectedItem.ToString();
            }
            try
            {
                using (var conn = new SqlConnection(connectionString))
                using (var cmd = new SqlCommand())
                {
                    ...
                    if (currentShiftId == 0)
                    {
                        cmd.CommandText = "sp_InsertShift";
                        cmd.CommandType = CommandType.StoredProcedure;
                    }
                    else
                    {
                        cmd.CommandText = "sp_UpdateShift";
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@id", currentShiftId);
                    }
                    cmd.Parameters.AddWithValue("@employee_id", cmbEmployee.SelectedValue);
                    cmd.Parameters.AddWithValue("@shift_date", dtpDate.Value.Date);
                    cmd.Parameters.AddWithValue("@start_time", st);
                    cmd.Parameters.AddWithValue("@end_time", et);
                    cmd.Parameters.AddWithValue("@break_duration", br);
                    cmd.Parameters.AddWithValue("@overtime_hours", nudOverTime == null ? 0 : nudOverTime.Value);
        ...
        {
            // Refresh only data related to the Shift tab
            LoadEmployeesForShift();   // reload employee list for shift combobox
            LoadShiftsByDate();        // reload shifts for current selected date
            ClearShiftForm();          // reset form inputs
            SetShiftFormState(false);  // exit edit mode if any
        }

        private void btnCoverage_Click(object sender, EventArgs e)
        {
            // Recompute coverage for current date
            ComputeCoverage();
        }
        #endregion
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand())
                {
                    ...
                    if (isUserAccountAddMode)
                    {
                        ...
                        cmd.Parameters.AddWithValue("@employee_id", employeeId == DBNull.Value ? (object)DBNull.Value : employeeId);
                        if (chbIsActive != null) cmd.Parameters.AddWithValue("@is_active", chbIsActive.Checked);
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            cmd.Parameters.AddWithValue("@password_salt", DBNull.Value);
                        }
                    }
                    conn.Open();
                    cmd.ExecuteNonQuery();
        ...
        private void cmbRole4_SelectedIndexChanged(object sender, EventArgs e)
        {
